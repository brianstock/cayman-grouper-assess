...
#   LIME-fixed-k model (fit growth outside model)
#   Francis weighting of length data

dir <- here()
setwd(dir)
res_dir <- file.path(dir, "results","LIME_fixed_growth_francis")
...
library(tidyverse)
library(TMB)
compile("code/LIME.cpp","-O0 -g") # modified from LIME package
dyn.load("code/LIME.so")
...
# --------------------------------------------------------
# get Little Cayman specific growth pars from m2 (fig3_growth_REisland.R)
growth.pars <- readRDS("results/growth_REisland_LCpars.rds")
linf = growth.pars[1]
vbk = growth.pars[2]
t0 = growth.pars[3]
...
dat_growth_fit = cbind(dat_growth$Age, dat_growth$Length_cm)
fit_growth = FALSE # map growth pars to NA (use starting values)

cvi=10 # CVlen = 0.096 best
# for(cvi in 1:length(CVlen_vec)){ # use loop to do likelihood profile over CV_L
	...
	est_rdev_t[(n.yrs-3):n.yrs] = 0 # fix last 4 rdevs at 0 (not estimable based on age at maturity)
	...
	if(is.null(S50_up)) Upr[which(names(obj$par)=="log_S50_f")] <- tmblist$Parameters$log_linf
	...
	saveRDS(output, file.path(out_dir, paste0("output_CV",CVlen_vec[cvi],"_francis_1.rds")))

	if(!is.null(opt)){
		if(!is.na(opt)){
			png(paste0(plots_dir, "/fixed_francis_1_CV",CVlen_vec[cvi],"_LIME_plots.png"),res=300,units='in',width=11,height=5.5)
			plot_output(Inputs=output$Inputs, Report=output$Report, Sdreport=output$Sdreport, true_years=yrs,
				plot=c("ML","Rec","Ind","SB","Fish","Selex"), set_ylim=list("Ind" = c(0,9500), "Rec"=c(0,40000)), relative=FALSE)
			dev.off()

			png(paste0(fits_dir,"/fixed_francis_1_CV",CVlen_vec[cvi],"_LIME_LCfit.png"),width=10,height=7,units="in",res=300)
	...
	obj2 <- TMB::MakeADFun(data=tmblist[["Data"]], parameters=tmblist[["Parameters"]], random=tmblist[["Random"]], map=tmblist[["Map"]], inner.control=list(maxit=1e3), DLL="LIME", checkParameterOrder = FALSE)
	opt2 <- fit_tmb(obj2, n.newton=newtonsteps, do.sdrep=TRUE, do.check=TRUE)
	...
	output2$opt <- opt2
	output2$years <- yrs	
	saveRDS(output2, file.path(out_dir, paste0("output_CV",CVlen_vec[cvi],"_francis_2.rds")))

	if(!is.null(opt2)){
		png(paste0(plots_dir, "/fixed_francis_2_CV",CVlen_vec[cvi],"_LIME_plots.png"),res=300,units='in',width=11,height=5.5)
		...
		png(paste0(fits_dir,"/fixed_francis_2_CV",CVlen_vec[cvi],"_LIME_LCfit.png"),width=10,height=7,units="in",res=300)
		g <- plot_LCfits(Inputs=output2$Inputs, Report=output2$Report, n=TRUE, year_labels=yrs)
		print(g + theme_sleek() + theme(legend.position='none'))
		dev.off()

		stdres <- plot_stdresiduals(output2)
		png(paste0(residuals_dir,"/fixed_francis_2_CV",CVlen_vec[cvi],"_resid.png"),width=8,height=4,units="in",res=300)
		print(stdres[[1]])
		dev.off()
	}
