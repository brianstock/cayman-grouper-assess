...
if(all(is.null(Inputs))){
	...
	if(plot_type=="density"){
		p <- p + geom_histogram(aes(x=Length, y=..density..), color="black", fill="black", position="identity", binwidth=binwidth, alpha=0.6)
...
if(all(is.null(Report))==FALSE){
  ...
  if(length(unique(df_all$Model))>1){
	p <- ggplot(df_all) + 
		geom_ribbon(data=df_all %>% filter(Type=="Observed"), aes(x=Length, ymin=0, ymax=Proportion), fill="black", alpha=0.5) +
		# scale_fill_brewer(palette="Set1") +
