...
# LIME-integrated model (modified LIME to fit growth within the model)
# Currently set up to only run best model with CV_L = 0.096, sigmaI = 0.175, and sigmaC = 0.2
#   if likelihood profiles desired, see lines 164-169 to run commented out loops

dir <- here()
setwd(dir)
res_dir <- file.path(dir, "results","LIME_integrated_francis")
...
# store standardized residuals from all models
x <- c("Label","Year","Stdres","lo","hi","w","sigI","sigC")
df.resids <- data.frame(matrix(ncol=length(x), nrow=0))
colnames(df.resids) <- x	

# store NLL, sd(resid), and mean(abs(resid))
x <- c("w","sigI","sigC","NLL","sdres_index","sdres_length","mabs_index","mabs_length")
df <- data.frame(matrix(ncol=length(x), nrow=0))
colnames(df) <- x	

cvi=10
# for(cvi in 1:length(CVlen_vec)){ # use loop to do likelihood profile over CV_L
ci = 3 # sigmaC = 0.2 lowest NLL
# for(ci in 1:length(sigC_vec)){ # use loop to get NLL over sigmaC (Fig. S4)
sI = 4 # sigmaI = 0.175 lowest NLL
# for(sI in 1:length(sigI_vec)){ # use loop to get NLL over sigmaI (Fig. S4)
	output <- list()
	...
	# 2002 FSA catch goes in model year 2001 (before spawning #s 2002)
	...
	fixF <- c(1:2,5,6,15,16) # trial and error, fix F = 0 in years with very low estimated F to get convergence
	est_F_ft[1,fixF] = 0
	...
	LFdist=0 # multinomial for Francis weighting
	est_selex_f=TRUE
	vals_selex_ft=-1

	est_rdev_t = rep(1,n.yrs)
	est_rdev_t[(n.yrs-3):n.yrs] = 0 # fix last 4 rdevs at 0 (not estimable, fish don't mature and show up at SPAG until age 5+
	...
	obj <- TMB::MakeADFun(data=tmblist[["Data"]], parameters=tmblist[["Parameters"]], random=tmblist[["Random"]], map=tmblist[["Map"]], inner.control=list(maxit=1e3), DLL="LIME_integrated_REisland", checkParameterOrder = FALSE)
	...
	opt <- tryCatch(fit_tmb(obj, n.newton=newtonsteps, do.sdrep=TRUE, do.check=TRUE), error=function(x) NA)
	if(!is.na(opt)){
		...
		output$opt <- opt	
		output$years <- yrs	
		saveRDS(output, file.path(out_dir, paste0("output_CV",CVlen_vec[cvi],"_sigI",sigI_vec[sI],"_sigC",sigC_vec[ci],"_francis_1.rds")))

		if(!is.null(opt)){
			png(paste0(plots_dir, "/francis_1_CV",CVlen_vec[cvi],"_sigI",sigI_vec[sI],"_sigC",sigC_vec[ci],"_LIME_plots.png"),res=300,units='in',width=11,height=5.5)
			...
			png(paste0(fits_dir,"/francis_1_CV",CVlen_vec[cvi],"_sigI",sigI_vec[sI],"_sigC",sigC_vec[ci],"_LIME_LCfit.png"),width=10,height=7,units="in",res=300)
		...
		# step 2, reweight ala Francis 2011 TA1.8
		lbins <- as.numeric(colnames(tmblist$Data$LF_tlf[,,1]))
		y <- apply(tmblist$Data$LF_tlf[,,1], 1, function(x) sum(x*lbins)/sum(x))
		ind.NA <- which(is.nan(y))
		y[ind.NA] <- NA
		yhat <- output$Report$ML_ft_hat[1,]
		yhat[ind.NA] <- NA
		Eby <- output$Report$plb[,,1]
		vy <- apply(Eby, 1, function(x) sum(tmblist$Data$lbmids^2 * x) - sum(tmblist$Data$lbmids * x)^2)
		w = 1/var((y - yhat)/sqrt(vy/tmblist$Data$n_LF_ft[1,]), na.rm=T)
		tmblist$Data$n_LF_ft[1,] = w * tmblist$Data$n_LF_ft[1,]

		output2 <- list()
		output2$Inputs <- tmblist
		obj2 <- TMB::MakeADFun(data=tmblist[["Data"]], parameters=tmblist[["Parameters"]], random=tmblist[["Random"]], map=tmblist[["Map"]], inner.control=list(maxit=1e3), DLL="LIME_integrated_REisland", checkParameterOrder = FALSE)
		opt2 <- tryCatch(fit_tmb(obj2, n.newton=newtonsteps, do.sdrep=TRUE, do.check=TRUE), error=function(x) NA)
		if(!is.na(opt2)){
			output2$Report <- tryCatch( obj2$report(), error=function(x) NA)
			output2$Sdreport <- tryCatch(sdreport(obj2, bias.correct=TRUE), error=function(x) NA )		
			output2$obj <- obj2
			output2$opt <- opt2
			output2$years <- yrs	
			saveRDS(output2, file.path(out_dir, paste0("output_CV",CVlen_vec[cvi],"_sigI",sigI_vec[sI],"_sigC",sigC_vec[ci],"_francis_2.rds")))

			if(!is.null(opt2)){
				png(paste0(plots_dir, "/francis_2_CV",CVlen_vec[cvi],"_sigI",sigI_vec[sI],"_sigC",sigC_vec[ci],"_LIME_plots.png"),res=300,units='in',width=11,height=5.5)
				plot_output(Inputs=output2$Inputs, Report=output2$Report, Sdreport=output2$Sdreport, true_years=yrs,
					plot=c("ML","Rec","Ind","SB","Fish","Selex"), set_ylim=list("Ind" = c(0,9500), "Rec"=c(0,40000)), relative=FALSE, est=list(Rec=est_rdev_t, Fish=est_F_ft))
				dev.off()

				png(paste0(fits_dir,"/francis_2_CV",CVlen_vec[cvi],"_sigI",sigI_vec[sI],"_sigC",sigC_vec[ci],"_LIME_LCfit.png"),width=10,height=7,units="in",res=300)
				g <- plot_LCfits(Inputs=output2$Inputs, Report=output2$Report, n=TRUE, year_labels=yrs)
				print(g + theme_sleek() + theme(legend.position='none'))
				dev.off()

				if(!is.na(output2$opt$na_sdrep)) if(!output2$opt$na_sdrep){
					stdres <- plot_stdresiduals(output2)
					png(paste0(residuals_dir,"/francis_2_CV",CVlen_vec[cvi],"_sigI",sigI_vec[sI],"_sigC",sigC_vec[ci],"_resid.png"),width=8,height=4,units="in",res=300)
					print(stdres[[1]])
					dev.off()

					stdres[[2]]$w <- w
					stdres[[2]]$sigI <- sigI_vec[sI]
					stdres[[2]]$sigC <- sigC_vec[ci]
					df.resids <- rbind(df.resids, stdres[[2]])

					if( all(c("par","objective") %in% names(output2$opt)) ) nll = output2$opt[["objective"]]
					if( all(c("par","value") %in% names(output2$opt)) ) nll = output2$opt[["value"]]
					if("opt" %in% names(output2$opt)) nll = output2$opt$opt$objective

					tmp.ind <- filter(stdres[[2]], Label=="Index") %>% summarize(sd=sd(Stdres), mabs=mean(abs(Stdres)))
					tmp.len <- filter(stdres[[2]], Label=="Mean length") %>% summarize(sd=sd(Stdres), mabs=mean(abs(Stdres)))
					x <- c("w","sigI","sigC","NLL","sdres_index","sdres_length","mabs_index","mabs_length")
					tmp <- data.frame(w=w, sigI=sigI_vec[sI], sigC=sigC_vec[ci], NLL=nll, 
						sdres_index=tmp.ind[1], sdres_length=tmp.len[1], mabs_index=tmp.ind[2], mabs_length=tmp.len[2])
					colnames(tmp) <- x
					df <- rbind(df, tmp)
				}
...
# only if loop over sigI and sigC
# # Figure S4: which values of sigI and sigC give lowest NLL
# png(file.path(res_dir,paste0("sigI_sigC_NLL.png")),width=5,height=4,units='in',res=300)
# print(ggplot(df, aes(x = sigI, y = sigC, color = NLL)) +
# 	geom_point(size=5) +
# 	scale_color_viridis(direction = -1) +
# 	xlab(expression(sigma[Index])) +
# 	ylab(expression(sigma[Catch])) +
# 	theme_sleek() +
