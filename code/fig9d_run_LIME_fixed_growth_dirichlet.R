...
#   Dirichlet for length composition (LIME default) 

dir <- here()
setwd(dir)
res_dir <- file.path(dir, "results","LIME_fixed_growth_dirichlet")
...
# for(cvi in 1:length(CVlen_vec)){ # use loop to do likelihood profile over CV_L
	...
	saveRDS(output, file.path(out_dir, paste0("output_CV",CVlen_vec[cvi],".rds")))

	if(!is.null(opt)){
		if(!is.na(opt)){
			png(paste0(plots_dir, "/fixed_dirichlet_CV",CVlen_vec[cvi],"_LIME_plots.png"),res=300,units='in',width=11,height=5.5)
			...
			png(paste0(fits_dir,"/fixed_dirichlet_CV",CVlen_vec[cvi],"_LIME_LCfit.png"),width=10,height=7,units="in",res=300)
